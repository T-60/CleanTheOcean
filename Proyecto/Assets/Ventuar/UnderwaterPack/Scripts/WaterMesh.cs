...
namespace LowPolyUnderwaterPack
{
	...
	public class WaterMesh : MonoBehaviour
	{
        ...
        [Tooltip("The default queue which particles render on when above water. Queue changes when above water so that the particles can be seen when looking through the water.")]
        [SerializeField] private int surfaceParticleQueue = 1999;
		[Tooltip("The queue which particles should render on when underwater.")]
        [SerializeField] private int underwaterParticleQueue = 3001;
		...
		// Used to make the main directional light's shadow maps accessible globally
		private CommandBuffer commandBuffer;
        private Light lightComponent;
		private ParticleSystemRenderer[] underwaterParticles = new ParticleSystemRenderer[0];
		...
		private void Start()
		{
			...
			// Find all underwater particle systems
			ParticleSystemRenderer[] particles = FindObjectsOfType<ParticleSystemRenderer>();
			List<ParticleSystemRenderer> underwaterParticlesList = new List<ParticleSystemRenderer>();
			for (int i = 0; i < particles.Length; i++)
			{
				Vector3 particlePosition = particles[i].transform.position;
				if (IsInUnderwaterBounds(particlePosition)) {
					
					// Add particle system to underwater particle array
					underwaterParticlesList.Add(particles[i]);
				}
			}

			underwaterParticles = underwaterParticlesList.ToArray();
        ...
        {
            if (lightComponent != null) lightComponent.RemoveCommandBuffer(LightEvent.AfterShadowMap, commandBuffer);
		...
		private void UpdatePropBlock()
		{
			...
			propBlock.SetFloat("_VisualizeWaves", visualizeWaves ? 1 : 0);
			propBlock.SetFloat("_VisualizeBaseWaves", visualizeBaseWaves ? 1 : 0);
			propBlock.SetFloat("_VisualizeWaveNoise", visualizeWaveNoise ? 1 : 0);
			propBlock.SetFloat("_VisualizeWaveFoam", visualizeWaveFoam ? 1 : 0);
			...
			propBlock.SetTexture("_FoamNoise", foamNoise ? foamNoise : null);
			...
			propBlock.SetTexture("_FoamDistortion", foamDistortionTexture ? foamDistortionTexture : null);
        ...
        private void SetupCommandBuffer()
        {
            ...
            commandBuffer.SetGlobalTexture("_SunCascadedShadowMap", shadowMapRenderTextureIdentifier);
		...
		private bool IsInUnderwaterBounds(Vector3 position) {
			return 	position.y < transform.position.y && 
					position.x < transform.position.x + (xSize * .5f) && position.x > transform.position.x - (xSize * .5f) &&
					position.z < transform.position.z + (zSize * .5f) && position.z > transform.position.z - (zSize * .5f);
		}

		private void UpdateParticleQueue() {
			...
			{
				int queue = underwaterEffect.isUnderwater ? underwaterParticleQueue : surfaceParticleQueue;

				// Check if the queue was already applied so the for loop doesn't run every frame
				if (underwaterParticles[0].material.renderQueue != queue)
				{
					// Loop through all particles and apply appropriate queue
					for (int i = 0; i < underwaterParticles.Length; i++)
					{
						underwaterParticles[i].material.renderQueue = queue;
					}
				}
			}
		}
	...
	public class WaterMesh_Editor : Editor
	{
		...
		private SerializedProperty computeShader, displayWireframe, visualizeWaves, visualizeBaseWaves, visualizeWaveNoise, visualizeWaveFoam, isEquilateral, xSize, zSize, subdivision, depthColor, shallowColor, foamColor, depthDistance, depthBlending,
			...
			waveAmplitude2, waveFrequency1, waveFrequency2, waveSpeed1, waveSpeed2, noiseAmplitude, noiseSpeed, noiseScale, surfaceParticleQueue, underwaterParticleQueue;
        ...
        private void OnEnable()
		{
            ...
            surfaceParticleQueue = serializedObject.FindProperty("surfaceParticleQueue");
            underwaterParticleQueue = serializedObject.FindProperty("underwaterParticleQueue");
		...
		public override void OnInspectorGUI()
		{
			...
			if (particleFoldout)
			{
				...
				EditorGUILayout.PropertyField(surfaceParticleQueue);
				EditorGUILayout.PropertyField(underwaterParticleQueue);
