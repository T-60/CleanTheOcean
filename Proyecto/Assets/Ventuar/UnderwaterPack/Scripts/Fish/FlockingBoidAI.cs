...
namespace LowPolyUnderwaterPack
{
    ...
    public class FlockingBoidAI : SingleFishAI
    {
        ...
        public Vector2 minMaxBoidGroup = new(1, 10);
        ...
        private bool useAlternateAlgorithm;
        ...
        protected override void Start()
        {
            ...
            // Should only apply if the fish was not spawned by a FishSpawner
            ...
            List<char> nameChars = new(name.ToCharArray());
        ...
        {
            useAlternateAlgorithm = true;

#if BURST_PRESENT
            // If not using jobs or boid master component does not exist, use alternate (slower) boids algorithm
            if (bm != null && bm.enabled && bm.runWithJobs) 
            {    
                // Using C# jobs algorithm
                // Velocity is handled by BoidMaster and the C# Jobs boids algorithm. All this needs to do is handle collision (does so further down the script).
                useAlternateAlgorithm = false;
            }
#endif
        ...
        /// <summary>
        /// Alternate boids algorithm for when the C# Jobs algorithm cannot be used. Generally muchslower than C# Jobs.
        /// </summary>
        private void AlternateBoidsAlgorithm()
        {
            ...
            else
            {
                ...
                for (int i = 0; i < nearBoidsLength && i < minMaxBoidGroup.y; i++)
                {
                    ...
                    if (!boids[i].TryGetComponent(out boid))
                        continue;
            ...
            headingForCollision = IsHeadingForCollision();
        }
