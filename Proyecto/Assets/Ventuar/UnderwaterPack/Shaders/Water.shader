...
Shader "Low Poly Underwater Pack/Water"
{
	Properties
	{
		/*
	...
	SubShader
	{
		...
		Pass
		{
			...
			float4 frag(v2f i) : SV_Target
			{
				...
				// Convert from 0-1 to -1-1 so that underside of water isn't completely covered in foam.
				float foamDepthDifference = abs(saturate(depthDifference / foamDistance) * 2 - 1);
				float surfaceNoiseCutoff = foamDepthDifference * _FoamNoiseCutoff;
				 ...
				 */
				float shadowAttenuation = GetSunShadowsAttenuation_PCF5x5(i.worldPosition, i.screenPosition.z, 0);
				...
				float3 specularReflection = _LightColor0.rgb * _SpecIntensity * specularLight * round(shadowAttenuation);
